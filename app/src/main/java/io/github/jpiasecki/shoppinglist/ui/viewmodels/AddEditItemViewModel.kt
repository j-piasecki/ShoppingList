...
import io.github.jpiasecki.shoppinglist.repositories.ItemsRepository
...
class AddEditItemViewModel @ViewModelInject constructor(
    ...
    fun addItemToList(listId: String, item: Item) {
        shoppingListsRepository.addItemToList(listId, item)
        itemsRepository.insert(item)
    }

    fun getAutoSetIcons() = config.getAutoSetIcons()

    suspend fun getForAutoComplete() = itemsRepository.getForAutoComplete()
}